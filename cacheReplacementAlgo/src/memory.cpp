...
namespace drama {
    ...
    uint8_t *Block::cache_attempt(size_t offset, size_t n) {
        int key = static_cast<int>(offset);
        int value = static_cast<int>(reinterpret_cast<intptr_t>(m_begin + offset));
        
        int cached_value = m_parent->cache.lilo_cache->get(key);
        if (cached_value != -1) {
            return reinterpret_cast<uint8_t*>(static_cast<intptr_t>(cached_value));
        }
        
        cached_value = m_parent->cache.lifo_cache->get(key);
        if (cached_value != -1) {
            return reinterpret_cast<uint8_t*>(static_cast<intptr_t>(cached_value));
        }
        
        // Try FILO cache (using separate LIFO instance)
        cached_value = m_parent->cache.filo_cache->get(key);
        if (cached_value != -1) {
            return reinterpret_cast<uint8_t*>(static_cast<intptr_t>(cached_value));
        }
        
        // Try SIEVE cache (using L3 cache)
        cached_value = m_parent->cache.sieve_cache->get(key);
        if (cached_value != -1) {
            return reinterpret_cast<uint8_t*>(static_cast<intptr_t>(cached_value));
        }
        
        // Data not in any cache, add it to all caches
        m_parent->cache.lilo_cache->put(key, value);
        m_parent->cache.lifo_cache->put(key, value);
        m_parent->cache.filo_cache->put(key, value);
        m_parent->cache.sieve_cache->put(key, value);
        
        // Still return nullptr as it's not yet in cache
        return nullptr;
    }
